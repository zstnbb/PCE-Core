...
| ⭐ **P1 已实测** (Claude Desktop, 2026-05-10) | **H2✓ + H3✓ + H4✗** | ✅ T1 | ✅ T1 | ❌ | ✅ | ✅ | **~94% T1** (A1+A2 双冗余) | ~95% | ~95% (H1 已交付) | **~94%** |
| ⭐ **P2 已实测** (ChatGPT Desktop, 2026-05-10) | **H2✓ + H3≈ + H4✗** | ✅ T1 | ≈ (推断 PASS，未独立验) | ❌ | ✅ (LocalCache 布局待发现) | ✅ | **~94% T1** (A1 主导) | N/A (P2 无 Cowork) | N/A (P2 无 Code) | **~94% (Chat-only)** |
...
- **C4 格式**：✅ **MAPPED 2026-05-10**（Claude Desktop v1.6608.2.0）。**重要修正**：ADR 先前假设 `local-agent-mode-sessions/` 是主要存储 — **不是**。实际那里只是「占位 / 锁 / 元数据」，嵌套为 `<account-uuid>/<task-uuid>/rpm`（1 byte placeholder，MSIX container ACL 锁外部进程读）+ `skills-plugin/<task-uuid>/<account-uuid>/` 反向索引。**真实会话状态在 `IndexedDB/https_claude.ai_0.indexeddb.leveldb/`** 里。用原生 .log 字节扫描 + ASCII run 提取在 1.19 MB log file 上某个 4297 个打印字符串 run，**Schema 底定**：
  - DB: `keyval-store` （主，622 变引出现）+ `claude-notifications` （副）。
  - Object stores 考古得出： `composer-drafts`（草稿，TipTap editor state JSON）、`conversations_v2:<epoch_ms>|anon`（会话，v2 schema）、`clientState`（全局状态）、`starredIds`（收藏）、`mutations` + `timestamp`（同步日志，CRDT-like）。
  - **TipTap editor state JSON 明文可读**：`{"state":{"tipTapEditorState":{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"<用户输入原文>"}]}]},"attachments":[],"files":[],"syncSourceUuids":[]},"version":1,"updatedAt":<epoch_ms>}` —含用户草稿原文（中文中面可读取）。
  - Conversation UUID 用 **UUID v7**（时间排序的 UUID，如 `019e0fdf-ec56-7e85-999c-cf0ec70b8914`）— 前 6 字节是毫秒时间戳，便于顺序扫描。
  - Assistant 消息提例名为 `new-assistant-message-uuid-<uuid>`。
  - Conversation 消息体：另外编码，UTF-16-LE bytes 在 leveldb。原生扫描看为 `(u7b`、`vpenc`、`bKNMR/f` 之类乱码，实为 CJK 字符 LE 字节被 Latin-1 误读的表象（例 `0x75 0x7B` 为 U+7528 = `用`）— v1 解析器需以 UTF-16-LE 编码读。
  - **敏感性警告**： TipTap drafts 含用户未发送草稿。L3g v1 读取该 store 必须取得用户明示同意，且 redact 选项默认为 `redact-drafts=true`。
  - L3g 状态跟踪错误修正价值：在 `pce_persistence_watcher/agent_sessions.py` v1 实现 IndexedDB LevelDB 读取、UTF-16-LE 解码、JSON 识别 + redact pipeline。 plyvel 不可用（Windows wheel） — 作为后续 Phase 7 考虑：纯 Python LevelDB 读取器（只读不写，可裁剪到仅支持 Chromium IndexedDB log/sst 格式）。
- **C5 格式**：❌ **OUT OF L3g SCOPE — mapped 2026-05-10**（Claude Desktop v1.6608.2.0）。原本 ADR 假设 `vm_bundles/` / `claude-code-vm/` 是「沙箱环境快照」，预期可以 envelope-level 抳。实际 dump 发现： `vm_bundles/claudevm.bundle/` 是**完整的 Linux VM 镜像包**：
  - `rootfs.vhdx` = **9.4 GB** Linux VHDX 根文件系统（+ `.zst` 压缩版 2.3 GB）
  - `initrd` = 177 MB initramfs（+ `.zst` 174 MB）
  - `vmlinuz` = 14 MB Linux kernel 镜像（+ `.zst`）
  - `sessiondata.vhdx` = 4 MB session-bound VHDX（这个可能是 Cowork agent 运行中产生的会话状态）
  - `smol-bin.vhdx` = 38 MB 辅助 binary VHDX
  - 六个 `.origin` 占位符（40 bytes 每个，可能是上游 download URL hash）
  - `claude-code/2.1.128/claude.exe` = **254 MB**（这才是 H1 / L3h 包装的目标）+ `.verified` 0-byte 完整性戳
  - `claude-code-vm/2.1.128/claude` = **248 MB**（VM 上下文里的 claude binary）
  - 总体积 ≈ 16 GB unique payload。读取需专门工具（`qemu-img convert`、`vhdxtool`、Hyper-V mount）。**L3g 不手抳该表面**：
    1. 资源过于重（占用 16 GB+ 磁盘 + 数十秒 IO time 才能简单 stat）。
    2. 敏感性：sessiondata.vhdx 中 session-bound VM 状态可能含用户代码 / API key / 其他不宜自动 ingest 的内容。
    3. 中间件起点不适：L3g 设计为「轻量封套抳」，VHDX 反序列化是 Tier-1 OS-级文件系统工作，不在 OSS 范围。
  - » **C5 从 v1.x L3g 范围中移除**，重新分类为「改造类 OS 工具」领域。未来考虑： Pro 频道 + 用户明示同意 + Hyper-V mount（需 admin）。
- **C4 「claude_desktop_config.json」 + 辅助 JSON 表面**：作为 C4 的补充，在 LocalCache 根 dump 到三个小 JSON 表面，全部明文且默认仅含元数据，L3g v1 可直接采集：
  - `claude_desktop_config.json` (442 bytes)： `preferences` 包含 `coworkScheduledTasksEnabled`、`ccdScheduledTasksEnabled`（CCD = Claude Code Desktop 推测）、`sidebarMode="chat"`、`coworkWebSearchEnabled`、`epitaxyPrefs.starred-cowork-spaces`（「epitaxy」看着是 Cowork 的内部代号）、`dframe-local-slice`等。
  - `cowork-enabled-cli-ops.json` (61 bytes)：只含 `ownerAccountId` (UUID) — 此为 Claude account UUID，与 `local-agent-mode-sessions/` 顶层路径 UUID 同源。
  - `git-worktrees.json` (41 bytes)： `{"worktrees": {}, "schemaVersion": 2}` — Cowork git worktree 状态， v1→v2 迁移过一次。
  - `ant-did` (48 bytes, base64-encoded UUID)：Anthropic device-id。例 `bb547b29-2f6c-4f3f-b88c-6e603132eaeb` — 可作为 PCE 设备关联的稳定 anchor，跨 Claude 升级保持。
  - » 这六个表面是「v1 低反股」采集点： JSON、小、明文、默认仅元数据。L3g v1 实现 `agent_sessions.py::scan_local_jsons()` 必须含这些。
- **ChatGPT Desktop 持久化底色**：仅作为参考钉在这里： 2026-05-10 dump，ChatGPT IndexedDB `https_chatgpt.com_0.indexeddb.leveldb/000003.log` **只有 3881 bytes**，表明 ChatGPT Desktop 实际 **几乎不本地缓存会话**。Schema：`webrtc-cert-db` (P2P TLS cert store, ECDSA-P256, 1 个月轮换有3±上下身份) + `ConversationsDatabase` (会话包装器, indices `id` / `accountUserId` / `authUserId`)。P2 L3g 在 LocalCache 抳到的主要是 WebRTC P2P cert 与 elective 会话索引，会话内容本体在服务端。» **P2 L3g 表面明显薄于 P1**，Chat 区总抳主要走 A1 mitmproxy（H2-P2 PASS 已使 A1 为 P2 Chat 区主导路径）。
- **F1 PCE-Logger Skill prototype**：写一个 markdown skill 测 Claude 是否会调用我们提示的 tool；如不调用，F1 降级为元数据-only 维度。
- **ChatGPT Desktop H2 同等性**：✅ **VALIDATED 2026-05-10**（ChatGPT Desktop v1.2026.119.0, AUMID `OpenAI.ChatGPT-Desktop_2p2nqsd0c76g0!ChatGPT`）。**方法**：`scripts/probe_h2_chatgpt.ps1`（与 P1 探针同构，仅换 AUMID + regex domain set 为 `openai.com|chatgpt.com|oaistatic.com|oaiusercontent.com|chat.openai.com|ws.chatgpt.com`）。**产出**：3.2 MB log，30s 观察期内 **361 个洁净 chatgpt.com / ws.chatgpt.com HTTP 命中**，388 个成功响应状态码分布：368×200 / 9×206 / 5×304 / 4×202 / 1×302 / 1×403，**0 个 TLS handshake / 证书错误**，**0 Client disconnected**，**0 真实 503**。Log 含真实应用 cookie（`oai-client-auth-info` JSON-encoded user payload + `_puid` user-bound 会话凭证）—表明客户端不仅 TCP 通，且接受 mitmproxy CA 签的 leaf cert 完成 TLS 握手 + 应用层身份认证。**结论**：OpenAI **不对 chatgpt.com / ws.chatgpt.com 做 cert pinning**（与社区 2025 年 pin 报告相反；可能 OpenAI 已静默 unpin、或 MSIX 渠道不 pin、或探针某种原因充其量）。» **A1 mitmproxy 路径在 P2 也 viable**；与 P1 并论，§3.6 的「已实测」场景在 P1 和 P2 两表上同样锁定。剩余 H4 推断 LOCKED（同 MSIX Electron Fuses 集），无需独立实测；H3 推断 PASS（同 Chromium 网络服务进程逻辑），不是 acceptance gate 的必需。
