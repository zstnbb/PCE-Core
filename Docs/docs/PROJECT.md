...
- Status: Active
- Current Phase: **P5.C Meta-Pipeline & Maintainability (v1.1.5)** — P5.B chat-region D0 释放门（19/22 PASS）已过；P5.B.5 cowork-region 与 P5.B.6 code-region 作为并行轨道 carry forward；**P5.C 2026-05-12 在契约冻结阶段 (P5.C.0)启动**，负责把跨 lane 抓取/验证/健康度三件套契约 + 治理产物落地（详见 §11 P5.C 行）。
- Updated: 2026-05-12
...
| **P5.B** 🟢 | v1.1 IDE & Desktop & MCP | **L3f MCP middleware 先行** ✅ + **L3d CDP launcher** ✅ (Cursor / Windsurf / Squirrel + macOS Claude Desktop；MSIX Claude Desktop 由 ADR-018 重路) + L3e CLI gateway (交给 L3h) + `.mcpb` Desktop Extensions 官方一键安装 ✅。**«新增» L3g 持久化观察 + L3h CLI wrap ✅ alpha.8（ADR-018 三主轴 axes 2 + 3）**。**P1 Claude Desktop chat-region D0 释放门 ✅（19/22 PASS, 2026-05-10）**。P5.B.5 cowork-region (17 C-cases standard-down done) + P5.B.6 code-region (E-cases standard-down done)作为并行轨道 carry forward。L3c VS Code 扩展**重排到 P5.D**（原 P5.C 位置被 Meta-Pipeline 占用）。L3b Electron preload 在 v1.1 不实施（ADR-016 延后，ADR-018 H4 LOCKED 锁死）。 | 4–6 周 |
| **P5.C** 🟢 | **v1.1.5 Meta-Pipeline & Maintainability** | **跨 lane 三件套契约 (capture/verification/health-as-data) 冻结 + 激活 ADR-017 Test Conductor + 激活 ADR-011 G3+G9** + 健康度 dashboard + nightly probe + auto-issue + 配置化适配 (3 S0 站 YAML 化) + AI 辅助 selector 修复 + 治理产物 (CONTRIBUTING/CODEOWNERS/templates) + 根目录 .diag_*.py 清理。针对项目所有者「无法主体维护」约束为项目提供可持续生存的工程基础。 | 6–8 周 |
| P5.D | v1.1.6 IDE plugin | L3c VS Code Extension API + Copilot research（原 P5.C 位置） | 3–4 周 |
| P6 | v1.2 Pinning-Proof / Coverage Polish | L2 Frida SSL hook（解锁 ChatGPT Desktop 等 pinned 客户端）+ L4b AX 提前 + S2/S3 站点重启可选 | 6–8 周 |
...
**P5.C 启动**（2026-05-12）：
- 决议记录：`docs/decisions/2026-05-12-meta-pipeline-and-maintenance-strategy.md` ⭐ **本轮决议** — 9 章，定义 P5.C 6 个 sub-phase + 4 项强制契约 + 3 项治理产物 + 4 个待拍板决策
- 实施框架：`docs/engineering/META-PIPELINE-FRAMEWORK.md` ⭐ **canonical** — 4 lane × 三件套契约（capture/verification/health-as-data）+ AdapterContract + Test Conductor manifest 规范
- 决策固化：`docs/engineering/adr/ADR-019-maintenance-as-first-class-concern.md` — 治理优于功能作为产品一等约束
- 验证契约：`stability/DESKTOP-D-CASE-EXECUTION-STANDARD.md` (chat-region D-case) + `stability/PCE-PIPELINE-HEALTH-MATRIX.md` (健康度矩阵)
- Kickoff handoff：`handoff/HANDOFF-META-PIPELINE-KICKOFF-2026-05-12.md` ⭐ **active** — P5.C.0 → P5.C.5 子阶段，每阶段含 deliverable / acceptance / non-goals
...
- Kickoff handoff：`handoff/HANDOFF-IDE-DESKTOP-KICKOFF.md` 🟡 **carry-on**（P5.B chat-region 已过 D0，cowork+code 余项与 P5.C 并行） — P5.B.0 → P5.B.6 子阶段，每阶段含可发布切片 / acceptance criteria / 模块落点 / 产品×子阶段 unlock 表
- chat-region 完工 handoff：`handoff/HANDOFF-P1-CLAUDE-DESKTOP-CHAT-FULL-SWEEP-2026-05-10.md` ✅ — Claude Desktop chat 19/22 PASS 闭环（5 个 sub-run 实测 evidence）
- cowork-region kickoff：`handoff/HANDOFF-P1-CLAUDE-DESKTOP-COWORK-KICKOFF-2026-05-10.md` 🟡 — standard-down done, impl 待启
...
- `stability/PCE-PROBE-E2E-EXECUTION-STANDARD.md` — **浏览器 T-case 执行标准**（2026-05-04，verification-as-data 契约之一）
...
- `handoff/HANDOFF-IDE-DESKTOP-KICKOFF.md` 🟡 — **carry-on**（P5.B chat-region 已过 D0，cowork+code 余项与 P5.C 并行）
- `handoff/HANDOFF-P1-CLAUDE-DESKTOP-CHAT-FULL-SWEEP-2026-05-10.md` ✅ — P1 chat-region 19/22 PASS 闭环
- `handoff/HANDOFF-P1-CLAUDE-DESKTOP-COWORK-KICKOFF-2026-05-10.md` 🟡 — cowork-region standard-down done, impl 待启
- `handoff/HANDOFF-META-PIPELINE-KICKOFF-2026-05-12.md` ⭐ — **当前 active（P5.C v1.1.5）**

## 13. 对后来 agent 的阅读顺序推荐

### 13.A 接手 **P5.C 阶段**的 agent（当前 active，~2.5 小时）

第一次接手 PCE P5.C 阶段的 agent，走以下顺序：

1. 本文件 `docs/PROJECT.md`（项目总边界、§11 阶段划分含 P5.C）
2. ⭐ `docs/decisions/2026-05-12-meta-pipeline-and-maintenance-strategy.md` — P5.C "**为什么**" 的真理来源、9 章。
3. ⭐ `docs/engineering/META-PIPELINE-FRAMEWORK.md` — P5.C "**怎么做**" 的工程级定义、4 lane × 三件套契约。
4. ⭐ `docs/engineering/adr/ADR-019-maintenance-as-first-class-concern.md` — 决策固化。
5. ⭐ `stability/DESKTOP-D-CASE-EXECUTION-STANDARD.md` + ⭐ `stability/PCE-PIPELINE-HEALTH-MATRIX.md` — P5.C "**什么算做完**" 的两个验证契约。
6. `docs/engineering/adr/ADR-017-test-conductor-cross-lane-agent-contract.md` — P5.C.2 实施依据（status: Proposed → Adopted）。
7. `docs/engineering/adr/ADR-011-probe-remote-agent-autonomous-testing.md` G3 + G9 — P5.C.3 + P5.C.4 实施依据。
8. ⭐ `handoff/HANDOFF-META-PIPELINE-KICKOFF-2026-05-12.md` — P5.C.0 → P5.C.5 子阶段执行入口。
9. `stability/PCE-PROBE-E2E-EXECUTION-STANDARD.md` — 浏览器奇妹篇（只读，不改）。

### 13.B 接手 **P5.B 余项**轨道的 agent（cowork-region / code-region，~2 小时）

1. 本文件
2. **`research/DESKTOP-CAPTURE-COGNITIVE-FRAMEWORK.md`** ⭐ — strategy 轴
3. **`stability/DESKTOP-PRODUCT-MATRIX.md`** ⭐ — validation 轴（含 D/C/E case 全集）
4. `handoff/HANDOFF-BROWSER-EXT-FREEZE-2026-05-08.md`
5. `handoff/HANDOFF-IDE-DESKTOP-KICKOFF.md` — carry-on
6. `handoff/HANDOFF-P1-CLAUDE-DESKTOP-COWORK-KICKOFF-2026-05-10.md` — cowork standard
7. ⭐ `docs/engineering/META-PIPELINE-FRAMEWORK.md` — 余项 lane 也需遵守三件套契约
8. `docs/engineering/UNIVERSAL-CAPTURE-STACK-DESIGN.md` — UCS 蓝图
9. `docs/engineering/adr/ADR-009`/`010`/`012`-`018` — P5.B 期间落地决策
