...
|---|---|:---:|
| **V-GREEN** | 对**真实产品**跑过端到端 **+ 被动 / 自动 / 完整对话抓取**（不依赖用户为每条消息做手动动作），有具体 `raw_captures` / `messages` 行数或 handoff 签字作为证据 | ✅ |
...
> **重要 (2026-05-15)**: clipboard (L4a) 因为是用户主动行为的副产品 (见 §1.1),
> 重分类为 V-AUX **不计入 ≥3 invariant**. 本表 "关键 wave" 列同步剔除了
> L4a 路径, 改用真正的被动腿 (L3d CDP / L3f MCP / L3b preload). 之前
> 几版表里把 L4a 当 leg 是误读, 修正于本 amendment.

| 场景 | pre-sprint | 2026-05-15 (W1 close) | Phase A target | Phase B target | final | 关键 wave (passive legs only) |
|---|:---:|:---:|:---:|:---:|:---:|---|
| F1 ChatGPT Web              | 2 | 2 | 3 | 3 | 3 | W4-T1 (L3d CDP) |
| F1 Claude Web               | 2 | 2 | 3 | 3 | 3 | W4-T2 (L3d CDP) |
| F1 Gemini Web               | 1 | **2** ✅ W1-T4 (L1) | 3 | 3 | 3 | W4-T3-alt **(L3d CDP)** |
| F1 GAS                      | 1 | 1 (W1-T5 via L3a fallback, L1 仍 hermetic) | 3 | 3 | 3 | L1 孤立 sweep + L3d CDP |
| F1 Grok Web                 | 1 | **2** ✅ W1-T6 (L1) | 3 | 3 | 3 | W4-T3-alt **(L3d CDP)** |
| F4 P1 Claude Desktop        | 2 | **3** ✅ W1-T2 retry (`pce_capture` PASS, pair_id=d502a3e9...) | 3 | 3 | 3 | (达标) + W1-T1 (proxy posture B for 4-leg buffer) |
| F4 P2 ChatGPT Desktop       | 1 | 1 | 1+A2 | 3 | 3 | W2 + **W7-T2 UIA** |
| F5 P3 Cursor                | 0 | 0 | 1 (MCP) | 3 | 3 | **W8-T1/T2/T3** (L1 protobuf + MCP + preload/L3d) |
| F5 P4 Windsurf              | 1 | 1 (W1-T7 deferred) | 3 | 3 | 3 | W1-T7 (L3f MCP) + W4-T5-alt **(L3d CDP)** |
| F5 P5 GitHub Copilot        | 0 | 0 | 0 | 3 | 3 | **W6-T1/T2/T3** (L1 + L3c + MCP/L3d) |
| F6 P6 Claude Code CLI       | 1 | **3** ✅ W1-T3 (L3h) + W4-T6 (L1) + L3g | 3 | 3 | 3 | (达标) |
| F6 P7 Codex CLI             | 0 | **2** ✅ W1-T8 + W1-T9 (W4-T7 L1 blocked: Rust TLS bypass) | 3 | 3 | 3 | W4-T7 (Rust proxy fix, 1-2d eng) |
| F6 P8 Gemini CLI            | 0 | **2** ✅ W1-T10 + W1-T11 + bug fix (W4-T8 L1 blocked: Node undici bypass) | 3 | 3 | 3 | W4-T8 (undici ProxyAgent fix, 1-2d eng) |
| **TOTAL ≥3 V-GREEN**        | **0/13** | **2/13** (P1 Claude Desktop, P6 Claude Code CLI) | **10/13** | **13/13** | **13/13** | — |
