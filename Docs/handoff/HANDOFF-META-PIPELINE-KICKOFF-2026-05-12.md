...
法律边界产物 (产物 4–5, ADR-019 §3.5 — 同 commit 追加):
- ✅ `@f:\INVENTION\You.Inc\PCE Core\Docs\legal\THREAT-MODEL.md` — per-layer 风险矩阵, 7 个法律理论的防御论据, L0/L2 永久不进 OSS 的硬约束
- ✅ `@f:\INVENTION\You.Inc\PCE Core\Docs\legal\CEASE-AND-DESIST-RESPONSE.md` — 24h triage 流程, vendor kill-switch 工程 procedure, P0–P3 escalation tree
- ✅ `@f:\INVENTION\You.Inc\PCE Core\PRIVACY.md` §11–14 — ToS 免责 / Trade Secret / DMCA Designated Agent / no-legal-advice
- ✅ `@f:\INVENTION\You.Inc\PCE Core\CONTRIBUTING.md` Rule 5 — 合规边界 (L2/L0 forbidden in OSS, 禁用营销词汇, vendor 隔离强制)
- ✅ `@f:\INVENTION\You.Inc\PCE Core\README.md` Legal Notice 段
...
**Acceptance**:
- [ ] 接手 agent 读完 Tier 1+2 (≤75 min) 能在 5 分钟内回答以下 5 题:
  ...
  - Q5: 收到 ChatGPT 法务发来的 cease-and-desist, 第一个 24h 该做什么 / 不该做什么? (答: `Docs/legal/CEASE-AND-DESIST-RESPONSE.md` §1 + §8)

**Non-goals**:
- 不动代码
- 不实现 health beacon 调用点 (那是 P5.C.1)
- 不实现 Test Conductor (那是 P5.C.2)
- 不引入 `.github/workflows/legal-keyword-scan.yml` 与 `scripts/check_vendor_isolation.py` (那是 P5.C.5 锁口)
- 不为 v1.0 launch 做 counsel review (那是独立工作, 在 v1.0 launch 前 P0 必做, 但不在 P5.C 范围)
